...
This repository contains **PixiJS skills** for AI coding agents. When editing or adding skills, follow these rules.
...
- **Skill directory name** must exactly match the `name` in that skill’s frontmatter (e.g. `skills/pixijs-events/` ↔ `name: pixijs-events`).
...
- Be concise; avoid restating general PixiJS docs. Focus on correct API usage, pitfalls, and v8 patterns.
